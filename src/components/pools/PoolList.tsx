import { Button, Col, Divider, Empty, Input, Modal, Radio, Row } from 'antd';
...
import {
  ConversionPool_OrderBy,
  OrderDirection,
  useConversionPoolsQuery,
...
function ListItem({ pool }: { pool: ConversionPool | undefined }) {
  ...
  const [depositToken, setDepositToken] = useState<AccountId>(
    pool ? pool.out_token : '',
  );
  const [depositAmount, setDepositAmount] = useState<
    READABLE_AMOUNT | undefined
  >(undefined);
  const [inTokenReadableBalance, setInTokenReadableBalance] =
    useState<string>('');
  const [outTokenReadableBalance, setOutTokenReadableBalance] =
    useState<string>('');
  if (pool) {
    ftGetTokenMetadata(pool.in_token).then((e) => {
      setInTokenReadableBalance(
        NearAmount.transfer_to_readable(pool.in_token_balance, e.decimals),
      );
      setOutTokenReadableBalance(
        NearAmount.transfer_to_readable(pool.out_token_balance, e.decimals),
      );
    });
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Col span={6}>
          {pool
            ? `${pool.in_token}: ${inTokenReadableBalance}\n${pool.out_token}: ${outTokenReadableBalance}`
            : 'Balance'}
        </Col>
        <Col span={3}>
          {pool ? `${`${pool.in_token_rate}:${pool.out_token_rate}`}` : 'ratio'}
        </Col>
        {pool ? (
          <Col span={4}>
            <img
      ...
      {pool ? (
        <Modal
          ...
          footer={null}
          onOk={() =>
            ConvertorContract.deposit_token_into_pool(
              depositToken,
              depositAmount!,
              pool.id,
            ).then((e) => e.call())
...
export function PoolList() {
  ...
  // useConversionPoolsQuery({variables: {limit}})
  const { data, loading, error } = useConversionPoolsQuery({
    client: client,
    variables: {
      limit: 1,
      offset: 0,
      orderBy: ConversionPool_OrderBy.InTokenBalance,
