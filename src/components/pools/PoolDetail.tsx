...
export function PoolDetail({pool}: {pool: ConversionPool}) {
  ...
  return <div>
    <div style={{padding: "20px",display: "flex", justifyContent: "space-around", alignItems: "center"}}>
      convert:
      <Input style={{width: "200px", marginInline: "10px"}} onChange={handleInputChange}/>
      {inToken?inToken.symbol:''}
      {inToken?<img src={inToken.icon}/>:''}
      {`balance:${NearAmount.transfer_to_readable(pool.in_token_balance,inToken?inToken.decimals:1)}`}
    </div>

    <SwapButton/>
    <div style={{padding: "20px",display: "flex", justifyContent: "space-around", alignItems: "center"}}>
      receive:
      <Input style={{width: "200px", marginInline: "10px"}} disabled={true} value={outValue}/>
      {outToken?outToken.symbol:''}
      {outToken?<img src={outToken.icon}/>:''}
      {`balance:${NearAmount.transfer_to_readable(pool.out_token_balance,outToken?outToken.decimals:1)}`}
    </div>
    <div>
      <Row>Ratio:</Row>
      <Row align={"middle"} justify={"center"} style={{margin: "5px"}}>
        {inToken?(<span>
          {inToken.symbol}
          <img src={inToken.icon} style={{height: "50%", margin: "2px"}}/>
          {pool.in_token_rate}
        </span>):''
        }
        :
        {outToken?(
          <span>
            {pool.out_token_rate}
            <img src={outToken.icon} style={{height: "50%", margin: "2px"}}/>
            {outToken.symbol}
          </span>):''}
      </Row>
